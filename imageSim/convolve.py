...
def convolve(image,psf,doPSF=True,edgeCheck=False):
    ...
    if s.find('0')>0:
        ...
        if c%2==1:
            c1 = c/2+1
        boxdslice = (slice(int(r1),datadim1+int(r1)),slice(int(c1),datadim2+int(c1)))
            # Python 3 - CJS added integer conversions for r1,c1
    ...
    if doPSF:
        ...
        if boxd.shape[0]==psf.shape[0]:
            ...
            c1 = c/2+1
            boxpslice = (slice(int(r1),psf.shape[0]+int(r1)),slice(int(c1),psf.shape[1]+int(c1)))
