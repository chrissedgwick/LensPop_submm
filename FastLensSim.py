'''
MODIFIED FOR SUBMM GALAXIES 

Mainly to display source unlensed flux and source lensed flux; selection is based on latter criterion
only; seeing/SN criteria applied in original code should be ignored.
...
from pylens import massmodel, pylens
...
class FastLensSim(SO,S2N):
    ...
    def Reset(self):
        ...
        self.totallensedflux={}     # cw added for submm
        self.fakeLens={}
        self.image={}
        self.sigma={}
        self.fakeResidual={}
...
#===========================================================================
    ...
    def setSourcePars(self,b,m,x,y,q,p,r,n=1,pixelunits=False,sourcenumber=1):
        ...
        self.totallensedflux[sourcenumber]={}  #cw added for submm
...
#===========================================================================
    ...
    def lensASource(self,sourcenumber,bands):
        ...
        for band in bands:
            ...
            unlensedtotalsrcflux= 10**(self.ms[sourcenumber][band])  # cw - adapted for submm as ms = logS directly read in from Cai data.
            sm[band]=srcmodel*unlensedtotalsrcflux
        
            if sm[band].max()>0:  # cw - this is lensed source flux as a pixel array
                self.totallensedsrcmag[sourcenumber][band]=-2.5*numpy.log10(sm[band].sum())+self.zeropoints[band]
                self.totallensedflux[sourcenumber][band]=sm[band].sum()  # cw - added in for submm to show total source lensed flux
            else:
                self.totallensedsrcmag[sourcenumber][band]=99
                self.totallensedflux[sourcenumber][band]=0     # cw added for submm
        return sm
