...
'''
 Adjusted for submm data; selection is by lensed source flux only.
 
 Seeing and SN criteria not relevant (so SeeingTest routine set to return True).
...
class SO():
    ...
    def SeeingTest(self,src,band):     # cw - amended to return True always for submm galaxies
        ''' if (self.bfac*(self.bl[src]))**2<(self.rfac*(self.rs[src]))**2+(self.seeing[band]/self.pixelsize)**2:
           # cw - corrected seeing test by moving bfac into squared term on LHS
            return False
        else:
            return True'''
        return True
