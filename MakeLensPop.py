...
'''
Amended for submm galaxies

loadsubmm() routine introduced in PopulationFunctions.py
...
class SourcePopulation(SourcePopulation_):
    def  __init__(self,D=None,reset=False,
                  bands=['F814W_ACS','g_SDSS','r_SDSS','i_SDSS','z_SDSS','Y_UKIRT'],population="submm"
                  ):
        self.bands=bands
        self.beginRedshiftDependentRelation(D,reset)
        self.loadsubmm() # - introduced submm routine
...
class LensSample():
    ...
    def  __init__(self,D=None,reset=False,zlmax=2,sigfloor=100,
                  bands=['F814W_ACS','g_SDSS','r_SDSS','i_SDSS','z_SDSS','Y_UKIRT'],sourcepop="submm"
                  ):   # removed cosmo argument as not needed with Astropy
        self.sourcepopulation=sourcepop
        if D==None:
            import distances
            D=distances.Distance() # cw removed 'cosmo=cosmo' argument as not needed with Astropy
    ...
    def Generate_Lens_Pop(self,N,firstod=1,nsources=1,prunenonlenses=True,save=True):
        ...
        while M>0:
            ...
            print(M,int(timeleft)," minutes left")
            if M>100000:
                n=100000   # batches of 100,000
            else:
                n=M*1
            M-=n
            zl,sigl,ml,rl,ql=self.L.drawLensPopulation(n)
            zs,ms,xs,ys,qs,ps,rs=self.S.drawSourcePopulation(n*nsources,sourceplaneoverdensity=firstod,returnmasses=False)  # return masses set to False so removed mstar, mhalo
    ...
    def LoadLensPop(self,j=0,sourcepopulation="submm"):
        f=open("idealisedlenses/lenspopulation_%s_%i.pkl"%(sourcepopulation,j),'rb')
        self.lens=pickle.load(f,encoding='iso-8859-1')
        f.close()
...
if __name__ == "__main__":
    ...
    L=LensSample(reset=False,sigfloor=100,sourcepop="submm") # cosmo argument not needed for Astropy
