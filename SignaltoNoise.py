...
Selection is based on lensed source flux only;
seeing/SN criteria applied in original code should be ignored.
...
class S2N():
    ...
    def SourceMetaData(self, criterion, SNcutA=15,magcut=3,SNcutB=[10,8]):
        self.mag={}
        self.msrc={}
        self.bestband={}
        self.passfail={}
        ...
        for src in self.sourcenumbers: # cw - re-written for submm
            self.resolved[src]={}
            SNr={}
            ...
            for band in self.bands:
                ...
                self.msrc[src][band]=self.totallensedflux[src][band] # for submm FLUX
            ...
            self.bestband[src],dummy = max(SNr.items(), key=lambda x:x[1])
            
            self.passfail[src]=False     

            try:  
                if self.msrc[src][band]>criterion: # the submm criterion 
                #if self.mag[src]>magcut:          # replaced by flux test in line above
                    self.passfail[src]=True
                    print('     lens found (with source >',criterion,'mJy at 500um): mag ',str(round(self.mag[src],2)))
